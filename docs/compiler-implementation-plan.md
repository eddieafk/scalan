...
- Match expressions accept either braces or a Scala 3 indentation-based case
  region. Leading-trivia columns enforce aligned sibling cases and allow nested
  matches to close at the enclosing case indentation without global indent and
  dedent tokens.
...
- The first Scala 3 contextual-abstraction slice supports named `given` values,
  ...
  result leave an ordinary generic method unspecialized, its partial
  substitutions seed inference from concrete visible context parameters or
  givens. Contextual evidence can therefore infer every type argument or complete
  the arguments left unresolved by values and expected results; ordinary type
  conflicts are retained rather than repaired by contextual search. Candidate
  combinations are replayed through normal evidence ranking and recursive
  materialization; a unique viable combination wins, while distinct viable
  combinations produce a focused ambiguity diagnostic. Search considers visible
  contextual parameters and named givens, prefers a matching contextual
...
- Scala 3 method context bounds preserve each bound type and optional `as`
  ...
  and non-trait classes.
- Scala 3 class context bounds desugar to contextual constructor parameters
  according to the same placement rules. Both explicit and value-inferred class
  type arguments specialize those evidence types before `new` reuses ordinary
  contextual search and recursive given-factory materialization. Named and
  hidden witnesses retain their contextual marker when stored as class fields,
  so class methods can access a named witness directly or resolve it through
  `summon`. Focused native coverage exercises explicit and inferred type
...
- Scala 3 rule-1 placement moves the complete generated witness group
  immediately before the first ordinary parameter whose type selects a named
  witness, such as `[A: Parser as parser](input: parser.Input)`. Generic
  inference aligns source arguments with non-contextual parameter slots, while
  typed evidence records its target parameter index. Calls and allocations
  insert materialized witnesses at those recorded positions, including a later
  explicit `using` clause. Focused native coverage exercises explicit and
  contextual-only type inference, path-dependent parameter types, method and
  class placement, trailing evidence, runtime execution, NIR ordering, and
  missing or ambiguous witnesses.
...
- Compiler-owned Scala 3 `summonFrom` accepts ordered type-ascription cases in
  ...
  Braced and Scala 3 indentation-based `inline match` expressions reuse the
  ordinary match lowering for compile-time Boolean, integer, Float, Double,
  String, Char, null, and exact module selectors. Indented sibling cases align
  to the first case, and a nested match closes when its indentation returns to
  the enclosing case level. Ordered scalar/String/Char/null literal cases,
  literal or singleton object alternatives, and a required final wildcard or
  binding are supported; each lowered case has distinct branch-selection
  metadata and only the selected body reaches NIR. A reducible
  ...
  Null patterns reduce from direct or stable nulls and reject only values known
  to be non-null, such as literals, constructed values, module singletons, and
  scalars; an arbitrary runtime reference remains unresolved. Selector bindings
  propagate the selector's compile-time scalar, String, or nullness value through
  their compiler-generated binding block, which lets a guard reference either
  an untyped or typed bound name without treating ordinary user blocks as
  constant expressions. Typed bindings retain widened call-site constants after
  their static type test and compiler-generated cast, including values passed
  through `Any`. Ordered short-circuiting means a guard on a
  ...
  and type-pattern alternatives can be reduced from a call-site static type.
  Alternatives either remain wildcards or consistently bind the same name in
  every arm; a shared binding receives the explicit union of the arm types and
  erases to their common runtime representation. Positive subtype
  ...
  text literals compare by decoded value.
  ...
  `T` in the concrete inline-application scope, restricts the intrinsic to an
  `inline match` case region, rejects pattern bindings because no term value
  exists, and diagnoses abstract call-site types that cannot select a branch.
  Expanded caller NIR contains only the selected body: the synthetic `$match`
  local, runtime type tests, and the intrinsic itself are absent. Focused
  coverage is isolated in `SmokeTests6.cpp` to preserve incremental harness
  build times and exercises reference/scalar/fallback selection, aliased and
  qualified forms, transparent-result refinement, guarded wildcard, singleton
  object, null patterns, braced and nested indentation-based syntax,
...
- Runtime ABI 62 completes the initial Int surface with
  ...
  Byte-order selection, wider primitive widths, slicing, duplication,
  read-only views, and bulk transfer remain later slices.
- Runtime ABI 63 extends relative multibyte access with the default big-endian
  `ByteBuffer.getLong(): Long` and `putLong(value): ByteBuffer`. Both require
  eight remaining bytes and advance position by exactly eight only after the
  complete access succeeds. Put remains fluent, and increasing position
  preserves an existing mark for a later reset.

  LLVM composes each Long from two portable big-endian Int accesses and splits
  writes the same way, retaining every bit without unaligned loads or
  host-order assumptions. The buffer validates all eight bytes before touching
  storage, so `BufferUnderflowException` and `BufferOverflowException` leave
  position and backing bytes unchanged.

  Byte-order selection, remaining primitive widths,
  slicing, duplication, read-only views, and bulk transfer remain later slices.
- Runtime ABI 64 completes the initial Long surface with
  `ByteBuffer.getLong(index): Long` and
  `putLong(index, value): ByteBuffer`. Both operations require
  `0 <= index <= limit - 8`, use the default big-endian order, and leave
  position and mark unchanged. Indexed put remains fluent.
...
- Runtime ABI 65 extends relative multibyte access with the default big-endian
  ...
  Byte-order selection, remaining primitive widths,
  slicing, duplication, read-only views, and bulk transfer remain later slices.
- Runtime ABI 66 completes the initial Float surface with
  `ByteBuffer.getFloat(index): Float` and
  `putFloat(index, value): ByteBuffer`. Both operations require
  `0 <= index <= limit - 4`, reuse the raw IEEE-754 big-endian helpers, and
  ...
  Byte-order selection, remaining primitive widths, slicing, duplication,
  read-only views, and bulk transfer remain later slices.
- Runtime ABI 67 extends relative multibyte access with the default big-endian
  `ByteBuffer.getDouble(): Double` and `putDouble(value): ByteBuffer`. Both
  require eight remaining bytes and advance position by exactly eight only
  after the complete access succeeds. Put remains fluent, and increasing
  position preserves an existing mark for a later reset.

  LLVM reuses the portable big-endian Long helpers and bitcasts between the raw
  64-bit payload and `Double`, preserving every IEEE-754 bit without numeric
  conversion, unaligned loads, or host-order assumptions. Full-range validation
  ensures `BufferUnderflowException` and `BufferOverflowException` leave
  position and backing bytes unchanged.

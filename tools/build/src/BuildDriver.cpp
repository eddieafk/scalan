...
constexpr std::string_view CompilerVersion = "scalanative-0.1.0";
constexpr std::string_view NirFormatVersion = "nir-text-v4";
...
bool validateLinkerSelection(const NativeToolchain& toolchain,
  ...
  diagnostics.error(support::SourceSpan::none(),
                    ...
                    "SCALANATIVE_LLD to an explicit linker path");
  return false;
}
...
bool validateNativeToolchainCapabilities(const BuildOptions& options,
  ...
  if (!toolchain) {
    diagnostics.error(support::SourceSpan::none(),
                      ...
                      "SCALANATIVE_CLANG to an explicit clang path");
    return false;
  }

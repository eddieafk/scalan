...
std::filesystem::path toolLogPath(const std::string& command) {
  ...
  return std::filesystem::temp_directory_path() /
         ("scalanative-toolchain-" + std::to_string(hash) + ".log");
}
...
std::optional<std::filesystem::path> discoverLld() {
  if (const char* explicitLld = std::getenv("SCALANATIVE_LLD")) {
...
std::optional<NativeToolchain> discoverNativeToolchain() {
  if (const char* explicitClang = std::getenv("SCALANATIVE_CLANG")) {
...
NativeLinkPlan probeNativeLinkPlan(const NativeToolchain& toolchain,
  ...
  const std::filesystem::path objectPath =
      temporary / ("scalanative-link-probe-" + std::to_string(nonce) + ".o");
  const std::filesystem::path binaryPath =
      temporary / ("scalanative-link-probe-" + std::to_string(nonce));
  {

...
constexpr std::size_t ByteBufferSize = 40;
...
void emitRuntimeTypeHelpers(std::ostringstream& out,
  ...
  out << "define internal i32 @__scalanative_native_bytes_get_int(ptr %array, i32 "
         "%index, i1 %little_endian) {\n";
  out << "entry:\n";
  out << "  %high = call i16 @__scalanative_native_bytes_get_short(ptr %array, i32 "
         "%index, i1 false)\n";
  out << "  %low_index = add i32 %index, 2\n";
  out << "  %low = call i16 @__scalanative_native_bytes_get_short(ptr %array, i32 "
         "%low_index, i1 false)\n";
  out << "  %wide_high = zext i16 %high to i32\n";
  out << "  %wide_low = zext i16 %low to i32\n";
  out << "  %shifted_high = shl i32 %wide_high, 16\n";
  out << "  %big = or i32 %shifted_high, %wide_low\n";
  out << "  %little = call i32 @llvm.bswap.i32(i32 %big)\n";
  out << "  %value = select i1 %little_endian, i32 %little, i32 %big\n";
  out << "  ret i32 %value\n";
  out << "}\n\n";

  out << "define internal void @__scalanative_native_bytes_put_int(ptr %array, i32 "
         "%index, i32 %value, i1 %little_endian) {\n";
  out << "entry:\n";
  out << "  %little = call i32 @llvm.bswap.i32(i32 %value)\n";
  out << "  %encoded = select i1 %little_endian, i32 %little, i32 %value\n";
  out << "  %shifted = lshr i32 %encoded, 16\n";
  out << "  %high = trunc i32 %shifted to i16\n";
  out << "  %low = trunc i32 %encoded to i16\n";
  ...
  out << "  call void @__scalanative_native_bytes_put_short(ptr %array, i32 "
         "%low_index, i16 %low, i1 false)\n";
  out << "  ret void\n";
  out << "}\n\n";

  out << "define internal i64 @__scalanative_native_bytes_get_long(ptr %array, i32 "
         "%index, i1 %little_endian) {\n";
  out << "entry:\n";
  out << "  %high = call i32 @__scalanative_native_bytes_get_int(ptr %array, i32 "
         "%index, i1 false)\n";
  out << "  %low_index = add i32 %index, 4\n";
  out << "  %low = call i32 @__scalanative_native_bytes_get_int(ptr %array, i32 "
         "%low_index, i1 false)\n";
  out << "  %wide_high = zext i32 %high to i64\n";
  out << "  %wide_low = zext i32 %low to i64\n";
  out << "  %shifted_high = shl i64 %wide_high, 32\n";
  out << "  %big = or i64 %shifted_high, %wide_low\n";
  out << "  %little = call i64 @llvm.bswap.i64(i64 %big)\n";
  out << "  %value = select i1 %little_endian, i64 %little, i64 %big\n";
  out << "  ret i64 %value\n";
  out << "}\n\n";

  out << "define internal void @__scalanative_native_bytes_put_long(ptr %array, i32 "
         "%index, i64 %value, i1 %little_endian) {\n";
  out << "entry:\n";
  out << "  %little = call i64 @llvm.bswap.i64(i64 %value)\n";
  out << "  %encoded = select i1 %little_endian, i64 %little, i64 %value\n";
  out << "  %shifted = lshr i64 %encoded, 32\n";
  out << "  %high = trunc i64 %shifted to i32\n";
  out << "  %low = trunc i64 %encoded to i32\n";
  ...
  out << "  call void @__scalanative_native_bytes_put_int(ptr %array, i32 "
         "%low_index, i32 %low, i1 false)\n";
  out << "  ret void\n";
  out << "}\n\n";

  out << "define internal float @__scalanative_native_bytes_get_float(ptr %array, "
         "i32 %index, i1 %little_endian) {\n";
  out << "entry:\n";
  out << "  %bits = call i32 @__scalanative_native_bytes_get_int(ptr %array, i32 "
         "%index, i1 %little_endian)\n";
  out << "  %value = bitcast i32 %bits to float\n";
  out << "  ret float %value\n";
  out << "}\n\n";

  out << "define internal void @__scalanative_native_bytes_put_float(ptr %array, "
         "i32 %index, float %value, i1 %little_endian) {\n";
  out << "entry:\n";
  out << "  %bits = bitcast float %value to i32\n";
  out << "  call void @__scalanative_native_bytes_put_int(ptr %array, i32 %index, "
         "i32 %bits, i1 %little_endian)\n";
  out << "  ret void\n";
  out << "}\n\n";

  out << "define internal double @__scalanative_native_bytes_get_double(ptr %array, "
         "i32 %index, i1 %little_endian) {\n";
  out << "entry:\n";
  out << "  %bits = call i64 @__scalanative_native_bytes_get_long(ptr %array, i32 "
         "%index, i1 %little_endian)\n";
  out << "  %value = bitcast i64 %bits to double\n";
  out << "  ret double %value\n";
  out << "}\n\n";

  out << "define internal void @__scalanative_native_bytes_put_double(ptr %array, "
         "i32 %index, double %value, i1 %little_endian) {\n";
  out << "entry:\n";
  out << "  %bits = bitcast double %value to i64\n";
  out << "  call void @__scalanative_native_bytes_put_long(ptr %array, i32 %index, "
         "i64 %bits, i1 %little_endian)\n";
  out << "  ret void\n";
  out << "}\n\n";
  ...
  out << "  %order_slot = getelementptr i8, ptr %buffer, i64 " << ByteBufferOrderOffset
      << "\n";
  out << "  %order = load i8, ptr %order_slot\n";
  out << "  %little_endian = icmp ne i8 %order, 0\n";
  out << "  ret i1 %little_endian\n";
  out << "}\n\n";

  out << "define internal i1 @__scalanative_byte_buffer_order(ptr %buffer) {\n";
  out << "entry:\n";
  ...
  out << "define internal ptr @__scalanative_byte_buffer_set_order(ptr %buffer, i1 "
         "%little_endian) {\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %order_slot = getelementptr i8, ptr %buffer, i64 " << ByteBufferOrderOffset
      << "\n";
  out << "  %order = zext i1 %little_endian to i8\n";
  out << "  store i8 %order, ptr %order_slot\n";
  out << "  ret ptr %buffer\n";
  out << "}\n\n";
  ...
  out << "define internal i32 @__scalanative_byte_buffer_get_int(ptr %buffer) {\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %position_slot = getelementptr i8, ptr %buffer, i64 "
      << ByteBufferPositionOffset << "\n";
  ...
  out << "  %limit = load i32, ptr %limit_slot\n";
  out << "  %remaining = sub i32 %limit, %position\n";
  out << "  %has_int = icmp sge i32 %remaining, 4\n";
  out << "  br i1 %has_int, label %read, label %underflow\n";
  out << "read:\n";
  out << "  %backing_slot = getelementptr i8, ptr %buffer, i64 "
      << ByteBufferBackingOffset << "\n";
  out << "  %array = load ptr, ptr %backing_slot\n";
  ...
  out << "define internal ptr @__scalanative_byte_buffer_put_int(ptr %buffer, i32 "
         "%value) {\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %position_slot = getelementptr i8, ptr %buffer, i64 "
  ...
  out << "  %remaining = sub i32 %limit, %position\n";
  out << "  %has_int = icmp sge i32 %remaining, 4\n";
  out << "  br i1 %has_int, label %write, label %overflow\n";
  out << "write:\n";
  out << "  %backing_slot = getelementptr i8, ptr %buffer, i64 "
      << ByteBufferBackingOffset << "\n";
  out << "  %array = load ptr, ptr %backing_slot\n";
  ...
  out << "define internal i64 @__scalanative_byte_buffer_get_long(ptr %buffer) {\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %position_slot = getelementptr i8, ptr %buffer, i64 "
      << ByteBufferPositionOffset << "\n";
  ...
  out << "  %limit = load i32, ptr %limit_slot\n";
  out << "  %remaining = sub i32 %limit, %position\n";
  out << "  %has_long = icmp sge i32 %remaining, 8\n";
  out << "  br i1 %has_long, label %read, label %underflow\n";
  out << "read:\n";
  out << "  %backing_slot = getelementptr i8, ptr %buffer, i64 "
      << ByteBufferBackingOffset << "\n";
  out << "  %array = load ptr, ptr %backing_slot\n";
  ...
  out << "define internal ptr @__scalanative_byte_buffer_put_long(ptr %buffer, i64 "
         "%value) {\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %position_slot = getelementptr i8, ptr %buffer, i64 "
  ...
  out << "  %remaining = sub i32 %limit, %position\n";
  out << "  %has_long = icmp sge i32 %remaining, 8\n";
  out << "  br i1 %has_long, label %write, label %overflow\n";
  out << "write:\n";
  out << "  %backing_slot = getelementptr i8, ptr %buffer, i64 "
      << ByteBufferBackingOffset << "\n";
  out << "  %array = load ptr, ptr %backing_slot\n";
  ...
  out << "define internal float @__scalanative_byte_buffer_get_float(ptr %buffer) "
         "{\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %position_slot = getelementptr i8, ptr %buffer, i64 "
  ...
  out << "  %limit = load i32, ptr %limit_slot\n";
  out << "  %remaining = sub i32 %limit, %position\n";
  out << "  %has_float = icmp sge i32 %remaining, 4\n";
  out << "  br i1 %has_float, label %read, label %underflow\n";
  out << "read:\n";
  out << "  %backing_slot = getelementptr i8, ptr %buffer, i64 "
      << ByteBufferBackingOffset << "\n";
  out << "  %array = load ptr, ptr %backing_slot\n";
  ...
  out << "define internal ptr @__scalanative_byte_buffer_put_float(ptr %buffer, "
         "float %value) {\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %position_slot = getelementptr i8, ptr %buffer, i64 "
  ...
  out << "  %remaining = sub i32 %limit, %position\n";
  out << "  %has_float = icmp sge i32 %remaining, 4\n";
  out << "  br i1 %has_float, label %write, label %overflow\n";
  out << "write:\n";
  out << "  %backing_slot = getelementptr i8, ptr %buffer, i64 "
      << ByteBufferBackingOffset << "\n";
  out << "  %array = load ptr, ptr %backing_slot\n";
  ...
  out << "define internal double @__scalanative_byte_buffer_get_double(ptr %buffer) "
         "{\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %position_slot = getelementptr i8, ptr %buffer, i64 "
  ...
  out << "  %limit = load i32, ptr %limit_slot\n";
  out << "  %remaining = sub i32 %limit, %position\n";
  out << "  %has_double = icmp sge i32 %remaining, 8\n";
  out << "  br i1 %has_double, label %read, label %underflow\n";
  out << "read:\n";
  out << "  %backing_slot = getelementptr i8, ptr %buffer, i64 "
      << ByteBufferBackingOffset << "\n";
  out << "  %array = load ptr, ptr %backing_slot\n";
  ...
  out << "define internal ptr @__scalanative_byte_buffer_put_double(ptr %buffer, "
         "double %value) {\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %position_slot = getelementptr i8, ptr %buffer, i64 "
  ...
  out << "  %remaining = sub i32 %limit, %position\n";
  out << "  %has_double = icmp sge i32 %remaining, 8\n";
  out << "  br i1 %has_double, label %write, label %overflow\n";
  out << "write:\n";
  out << "  %backing_slot = getelementptr i8, ptr %buffer, i64 "
      << ByteBufferBackingOffset << "\n";
  out << "  %array = load ptr, ptr %backing_slot\n";
  ...
  out << "define internal double @__scalanative_byte_buffer_get_double_at(ptr "
         "%buffer, i32 %index) {\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %limit_slot = getelementptr i8, ptr %buffer, i64 " << ByteBufferLimitOffset
  ...
  out << "define internal ptr @__scalanative_byte_buffer_put_double_at(ptr %buffer, "
  ...
  out << "define internal float @__scalanative_byte_buffer_get_float_at(ptr %buffer, "
         "i32 %index) {\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %limit_slot = getelementptr i8, ptr %buffer, i64 " << ByteBufferLimitOffset
      << "\n";
  out << "  %limit = load i32, ptr %limit_slot\n";
  out << "  %last_index = sub i32 %limit, 4\n";
  out << "  %nonnegative = icmp sge i32 %index, 0\n";
  out << "  %within_limit = icmp sle i32 %index, %last_index\n";
  out << "  %valid = and i1 %nonnegative, %within_limit\n";
  out << "  br i1 %valid, label %read, label %invalid\n";
  ...
  out << "define internal ptr @__scalanative_byte_buffer_put_float_at(ptr %buffer, "
  ...
  out << "define internal ptr @__scalanative_byte_buffer_put_int_at(ptr %buffer, i32 "
         "%index, i32 %value) {\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %limit_slot = getelementptr i8, ptr %buffer, i64 " << ByteBufferLimitOffset
  ...
  out << "define internal i16 @__scalanative_byte_buffer_get_short_at(ptr %buffer, "
         "i32 %index) {\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %limit_slot = getelementptr i8, ptr %buffer, i64 " << ByteBufferLimitOffset
      << "\n";
  out << "  %limit = load i32, ptr %limit_slot\n";
  out << "  %last_index = sub i32 %limit, 2\n";
  out << "  %nonnegative = icmp sge i32 %index, 0\n";
  out << "  %within_limit = icmp sle i32 %index, %last_index\n";
  out << "  %valid = and i1 %nonnegative, %within_limit\n";
  out << "  br i1 %valid, label %read, label %invalid\n";
  ...
  out << "define internal ptr @__scalanative_byte_buffer_put_short_at(ptr %buffer, "
         "i32 %index, i16 %value) {\n";
  out << "entry:\n";
  out << "  call void @__scalanative_byte_buffer_require(ptr %buffer)\n";
  out << "  %limit_slot = getelementptr i8, ptr %buffer, i64 " << ByteBufferLimitOffset
  ...
  void emit(std::ostringstream& out) const {
    ...
    for (const DebugFileInfo& file : files_) {
      ...
      out << '!' << file.compileUnitNode
          ...
          << file.fileNode << ", producer: \"scalanative\", isOptimized: "
...
std::string lowerCall(const nir::Value& value, const std::string& expectedType,
  ...
  const bool isRuntimeByteBufferGetShortAt =
      target == support::StdNames::RuntimeByteBufferGetShortAt;
  const bool isRuntimeByteBufferPutShortAt =
      target == support::StdNames::RuntimeByteBufferPutShortAt;
  const bool isRuntimeByteBufferGetInt =
      target == support::StdNames::RuntimeByteBufferGetInt;
  const bool isRuntimeByteBufferPutInt =
      target == support::StdNames::RuntimeByteBufferPutInt;
  const bool isRuntimeByteBufferGetIntAt =
      target == support::StdNames::RuntimeByteBufferGetIntAt;
  const bool isRuntimeByteBufferPutIntAt =
      target == support::StdNames::RuntimeByteBufferPutIntAt;
  const bool isRuntimeByteBufferGetLong =
  ...
  const bool isRuntimeByteBufferGetFloat =
      target == support::StdNames::RuntimeByteBufferGetFloat;
  const bool isRuntimeByteBufferPutFloat =
      target == support::StdNames::RuntimeByteBufferPutFloat;
  const bool isRuntimeByteBufferGetFloatAt =
      target == support::StdNames::RuntimeByteBufferGetFloatAt;
  const bool isRuntimeByteBufferPutFloatAt =
      target == support::StdNames::RuntimeByteBufferPutFloatAt;
  const bool isRuntimeByteBufferGetDouble =
      target == support::StdNames::RuntimeByteBufferGetDouble;
  const bool isRuntimeByteBufferPutDouble =
      target == support::StdNames::RuntimeByteBufferPutDouble;
  const bool isRuntimeByteBufferGetDoubleAt =
      target == support::StdNames::RuntimeByteBufferGetDoubleAt;
  const bool isRuntimeByteBufferPutDoubleAt =
  ...
  const bool isRuntimeByteBufferOrder =
  ...
  const bool isRuntimeByteBufferOperation =
      ...
      isRuntimeByteBufferGetShortAt || isRuntimeByteBufferPutShortAt ||
      ...
      isRuntimeByteBufferGetFloat || isRuntimeByteBufferPutFloat ||
      isRuntimeByteBufferGetFloatAt || isRuntimeByteBufferPutFloatAt ||
  ...
  if (isRuntimeByteBufferOperation) {
    const bool returnsBuffer =
        ...
        isRuntimeByteBufferPut || isRuntimeByteBufferPutAt ||
        isRuntimeByteBufferClear || isRuntimeByteBufferPutShort ||
        isRuntimeByteBufferFlip || isRuntimeByteBufferPutShortAt ||
    ...
    const bool returnsFloat =
        isRuntimeByteBufferGetFloat || isRuntimeByteBufferGetFloatAt;
    const bool returnsDouble =
        isRuntimeByteBufferGetDouble || isRuntimeByteBufferGetDoubleAt;
    ...
    if (isRuntimeByteBufferSetPosition || isRuntimeByteBufferSetLimit ||
        ...
        isRuntimeByteBufferGetLongAt || isRuntimeByteBufferGetFloatAt ||
        isRuntimeByteBufferGetDoubleAt) {
    ...
    } else if (isRuntimeByteBufferPutShortAt) {
      expectedParameters.push_back("Int");
      expectedParameters.push_back("Short");
    } else if (isRuntimeByteBufferPutIntAt) {
      expectedParameters.push_back("Int");
      expectedParameters.push_back("Int");
      ...
      expectedParameters.push_back("Int");
      expectedParameters.push_back("Long");
    } else if (isRuntimeByteBufferPutFloat) {
      expectedParameters.push_back("Float");
    } else if (isRuntimeByteBufferPutFloatAt) {
      expectedParameters.push_back("Int");
      expectedParameters.push_back("Float");
    } else if (isRuntimeByteBufferPutDouble) {
      expectedParameters.push_back("Double");
    } else if (isRuntimeByteBufferPutDoubleAt) {
      expectedParameters.push_back("Int");
      expectedParameters.push_back("Double");
    ...
    } else if (isRuntimeByteBufferGetShortAt) {
      helper = "__scalanative_byte_buffer_get_short_at";
    } else if (isRuntimeByteBufferPutShortAt) {
      helper = "__scalanative_byte_buffer_put_short_at";
    } else if (isRuntimeByteBufferGetInt) {
      helper = "__scalanative_byte_buffer_get_int";
    } else if (isRuntimeByteBufferPutInt) {
      helper = "__scalanative_byte_buffer_put_int";
    } else if (isRuntimeByteBufferGetIntAt) {
      helper = "__scalanative_byte_buffer_get_int_at";
    } else if (isRuntimeByteBufferPutIntAt) {
      helper = "__scalanative_byte_buffer_put_int_at";
    } else if (isRuntimeByteBufferGetLong) {
    ...
    } else if (isRuntimeByteBufferGetFloat) {
      helper = "__scalanative_byte_buffer_get_float";
    } else if (isRuntimeByteBufferPutFloat) {
      helper = "__scalanative_byte_buffer_put_float";
    } else if (isRuntimeByteBufferGetFloatAt) {
      helper = "__scalanative_byte_buffer_get_float_at";
    } else if (isRuntimeByteBufferPutFloatAt) {
      helper = "__scalanative_byte_buffer_put_float_at";
    } else if (isRuntimeByteBufferGetDouble) {
      helper = "__scalanative_byte_buffer_get_double";
    } else if (isRuntimeByteBufferPutDouble) {
      helper = "__scalanative_byte_buffer_put_double";
    } else if (isRuntimeByteBufferGetDoubleAt) {
      helper = "__scalanative_byte_buffer_get_double_at";
    } else if (isRuntimeByteBufferPutDoubleAt) {
      helper = "__scalanative_byte_buffer_put_double_at";
    } else if (isRuntimeByteBufferOrder) {
...
CodegenResult LlvmCodegen::emit(const linker::LinkedProgram& program,
  ...
  out << "; ModuleID = 'scalanative'\n";
  out << "; Runtime ABI = '" << runtime::runtimeAbiName() << "'\n";
  out << "source_filename = \"scalanative\"\n\n";
  ...
  out << "declare i32 @llvm.bswap.i32(i32)\n";

...
const frontend::TypedPolymorphicFunctionApplication*
polymorphicFunctionApplicationFor(const frontend::AstExpression& expression,
                                  const ValueContext& context) {
  if (context.polymorphicFunctionApplications == nullptr ||
      !expression.span.isValid()) {
    return nullptr;
  }
  auto application =
      std::find_if(context.polymorphicFunctionApplications->rbegin(),
                   context.polymorphicFunctionApplications->rend(),
                   [&](const frontend::TypedPolymorphicFunctionApplication& candidate) {
                     return candidate.span.source == expression.span.source &&
                            candidate.span.start == expression.span.start &&
                            candidate.span.length == expression.span.length;
                   });
...
std::string byteBufferRuntimeName(const frontend::AstExpression& expression,
  ...
  if (callee.text == support::StdNames::ByteBufferGetShort) {
    return std::string(expression.children.size() == 1
                           ? support::StdNames::RuntimeByteBufferGetShort
                           : support::StdNames::RuntimeByteBufferGetShortAt);
  }
  if (callee.text == support::StdNames::ByteBufferPutShort) {
    return std::string(expression.children.size() == 2
                           ? support::StdNames::RuntimeByteBufferPutShort
  ...
  if (callee.text == support::StdNames::ByteBufferGetLong) {
    return std::string(expression.children.size() == 1
                           ? support::StdNames::RuntimeByteBufferGetLong
                           : support::StdNames::RuntimeByteBufferGetLongAt);
  }
  if (callee.text == support::StdNames::ByteBufferPutLong) {
    return std::string(expression.children.size() == 2
                           ? support::StdNames::RuntimeByteBufferPutLong
                           : support::StdNames::RuntimeByteBufferPutLongAt);
  }
  if (callee.text == support::StdNames::ByteBufferGetFloat) {
    return std::string(expression.children.size() == 1
                           ? support::StdNames::RuntimeByteBufferGetFloat
                           : support::StdNames::RuntimeByteBufferGetFloatAt);
  }
  if (callee.text == support::StdNames::ByteBufferPutFloat) {
    return std::string(expression.children.size() == 2
                           ? support::StdNames::RuntimeByteBufferPutFloat
                           : support::StdNames::RuntimeByteBufferPutFloatAt);
  }
  if (callee.text == support::StdNames::ByteBufferGetDouble) {
    return std::string(expression.children.size() == 1
                           ? support::StdNames::RuntimeByteBufferGetDouble
                           : support::StdNames::RuntimeByteBufferGetDoubleAt);
  }
  if (callee.text == support::StdNames::ByteBufferPutDouble) {
    return std::string(expression.children.size() == 2
  ...
  if (callee.text == support::StdNames::ByteBufferSlice) {
    return std::string(expression.children.size() == 1
                           ? support::StdNames::RuntimeByteBufferSlice
                           : support::StdNames::RuntimeByteBufferSliceAt);
  }
...
bool isRequireConstCallee(const frontend::AstExpression& expression,
                          const ValueContext& context) {
  return isCompiletimeCallee(expression, context,
...
nir::Value valueFor(const frontend::AstExpression& expression,
    ...
    }
    {
      ...
      if (tupleOperation) {
        ...
        if (emptyTuple || concreteTuple) {
          ...
          const bool elementSelection =
              expression.text == support::StdNames::TupleHead ||
              expression.text == support::StdNames::TupleLast;
          if (elementSelection && arity != 0) {
            const std::size_t index =
                expression.text == support::StdNames::TupleHead ? 1 : arity;
              ...
              if (const std::string boxed = boxedObjectTypeName(resultType->kind);
                ...
                element =
                    nir::unboxValue(boxed, std::move(element), expression.span);
                         ...
                         runtime != "Object") {
                element = nir::asInstanceOfValue(
                    runtime, std::move(element), expression.span);
              }
            }
            values.push_back(std::move(element));
            return nir::blockValue(std::move(values), expression.span);
          }
          const bool tupleShrink =
              expression.text == support::StdNames::TupleTail ||
              expression.text == support::StdNames::TupleInit;
          if (tupleShrink && arity == 1) {
          ...
          if (tupleShrink && arity > 1) {
            ...
            if (resultType == nullptr ||
              ...
              return nir::unknownValue(
                  "<malformed-tuple-" + expression.text + ">", expression.span);
            }
            std::vector<nir::Value> elements;
            elements.reserve(arity - 1);
            const std::size_t firstIndex =
                expression.text == support::StdNames::TupleTail ? 2 : 1;
            const std::size_t lastIndex =
                expression.text == support::StdNames::TupleTail ? arity
                                                                 : arity - 1;
  ...
  case AstExpressionKind::Call: {
    ...
    if (const frontend::TypedPolymorphicFunctionApplication* application =
      ...
      invocation.reserve(application->hasReceiver ? 3 : 2);
      if (application->hasReceiver) {
        nir::Value receiver = expressionValueFor(application->receiver, context);
        receiver = narrowCompositeValue(std::move(receiver), application->receiverType,
                                        application->receiver, context);
        invocation.push_back(
            nir::localLetValue("this", runtimeTypeName(application->receiverType),
                               std::move(receiver), application->receiver.span));
    ...
    if (callCallee.kind == AstExpressionKind::Select &&
      ...
      const bool concreteTuple = receiverType != nullptr &&
                                 isTupleClassName(runtimeTypeName(*receiverType)) &&
                                 !receiverType->typeArguments.empty();
      if (application != nullptr && resultType != nullptr &&
          (emptyTuple || concreteTuple)) {
        const std::string receiverName =
            "tupleMap$receiver$" + std::to_string(expression.span.start);
        std::vector<nir::Value> evaluation;
        evaluation.push_back(nir::localLetValue(
        ...
        if (!isTupleClassName(runtimeTypeName(*resultType)) ||
            resultType->typeArguments.size() != receiverType->typeArguments.size() ||
            application->mappedResultTypes.size() !=
                receiverType->typeArguments.size()) {
          return nir::unknownValue("<malformed-polymorphic-tuple-map>",
                                   expression.span);
        }
        ...
        for (std::size_t index = 1; index <= receiverType->typeArguments.size();
          ...
          invocation.push_back(nir::localLetValue(
              application->parameterName, runtimeTypeName(application->parameterType),
              std::move(element), expression.children[1].span));
          invocation.push_back(scopedBodyValueFor(application->body, functionContext));
          nir::Value mapped =
              nir::blockValue(std::move(invocation), application->body.span);
          if (const std::string boxed =
                  boxedObjectTypeName(application->resultType.kind);
              !boxed.empty() && runtimeTypeName(application->resultType) != "Object") {
            mapped = nir::boxValue(boxed, std::move(mapped), expression.span);
          }
...
nir::Module NirEmitter::emit(const frontend::TypedModule& module) const {
  ...
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferPutAt),
                          ...
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferGetShort),
                          "(java.nio.ByteBuffer)Short", support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferPutShort),
                          "(java.nio.ByteBuffer,Short)java.nio.ByteBuffer",
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferGetShortAt),
                          "(java.nio.ByteBuffer,Int)Short",
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferPutShortAt),
                          "(java.nio.ByteBuffer,Int,Short)java.nio.ByteBuffer",
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferGetInt),
                          "(java.nio.ByteBuffer)Int", support::SourceSpan::none());
  ...
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferPutIntAt),
                          ...
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferGetLong),
                          "(java.nio.ByteBuffer)Long", support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferPutLong),
                          "(java.nio.ByteBuffer,Long)java.nio.ByteBuffer",
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferGetLongAt),
                          "(java.nio.ByteBuffer,Int)Long", support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferPutLongAt),
                          "(java.nio.ByteBuffer,Int,Long)java.nio.ByteBuffer",
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferGetFloat),
                          "(java.nio.ByteBuffer)Float", support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferPutFloat),
                          "(java.nio.ByteBuffer,Float)java.nio.ByteBuffer",
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferGetFloatAt),
                          "(java.nio.ByteBuffer,Int)Float",
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferPutFloatAt),
                          "(java.nio.ByteBuffer,Int,Float)java.nio.ByteBuffer",
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferGetDouble),
                          "(java.nio.ByteBuffer)Double", support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferPutDouble),
                          "(java.nio.ByteBuffer,Double)java.nio.ByteBuffer",
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferGetDoubleAt),
                          "(java.nio.ByteBuffer,Int)Double",
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferPutDoubleAt),
                          "(java.nio.ByteBuffer,Int,Double)java.nio.ByteBuffer",
                          support::SourceSpan::none());
  ...
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferSetOrder),
                          ...
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferSlice),
                          "(java.nio.ByteBuffer)java.nio.ByteBuffer",
                          support::SourceSpan::none());
  builder.addFunctionDecl(std::string(support::StdNames::RuntimeByteBufferSliceAt),
                          "(java.nio.ByteBuffer,Int,Int)java.nio.ByteBuffer",
                          support::SourceSpan::none());

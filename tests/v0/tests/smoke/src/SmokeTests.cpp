...
object InvalidMain {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-main-args";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-main-args.out";
  const std::filesystem::path emptyOutputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-main-args-empty.out";
...
object Invalid {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-uncaught-throw";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-uncaught-throw.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object InvalidArray {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-array-literals";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-array-literals.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-array-parameters";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-array-parameters.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object InvalidBooleanArray {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-boolean-arrays";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-boolean-arrays.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object InvalidLongArray {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-long-arrays";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-long-arrays.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object InvalidClassArray {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-class-arrays";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-class-arrays.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object InvalidTraitArray {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-trait-arrays";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-trait-arrays.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object InvalidDoubleArray {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-double-arrays";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-double-arrays.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object InvalidFloatArray {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-float-arrays";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-float-arrays.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object InvalidCharArray {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-char-arrays";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-char-arrays.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-arrays";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-arrays.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-mixed-arrays";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-mixed-arrays.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-parameters";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-any-parameters.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-local-any";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-local-any.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-fields";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-fields.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-ctors";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-ctors.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-unit-symbol";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-any-unit-symbol.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
int smokeBuildDriverActions() {
  ...
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-output.ll";
  std::filesystem::remove(outputPath);
  ...
  const std::filesystem::path optimizedLlvmPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-optimized-output.ll";
  const std::filesystem::path optimizationReportPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-optimization-report.json";
  ...
  const std::filesystem::path optimizedNirPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-optimized-output.nir";
  const std::filesystem::path optimizedNirReportPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-optimized-nir-report.json";
  ...
  const std::filesystem::path objectPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-output.o";
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-binary";
  std::filesystem::remove(binaryPath);
...
object PrintMain {
  ...
  const std::filesystem::path printBinaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-print";
  const std::filesystem::path printOutputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-print.out";
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-module-singletons";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-module-singletons.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-singleton-pattern";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-singleton-pattern.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-hybrid-memory-runtime";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-hybrid-memory-runtime.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object VirtualMethod {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-zone-scopes";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-zone-scopes.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-gc-telemetry";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-gc-telemetry.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-automatic-gc";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-automatic-gc.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-class-instance";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-class-instance.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-class-fields";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-class-fields.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-mutable-fields";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-mutable-fields.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-param-fields";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-param-fields.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main extends Labeled {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-inheritance-metadata";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-inheritance-metadata.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-runtime-type-tests";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-runtime-type-tests.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path failingBinaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-runtime-failing-cast";
  const std::filesystem::path failingOutputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-runtime-failing-cast.out";
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-short-circuit";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-short-circuit.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-remainder";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-remainder.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-unary-operators";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-unary-operators.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-string-concat";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-string-concat.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-string-equality";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-string-equality.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-equals";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-equals.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-equals";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-equals.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-string";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-string.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-unit-equals-hashcode";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-unit-equals-hashcode.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-unit-operator-equality";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-unit-operator-equality.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-hashcode";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-hashcode.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-string-length";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-string-length.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-primitive-tostring";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-primitive-tostring.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-tostring";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-any-tostring.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-custom-tostring";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-custom-tostring.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-formatted-interpolation";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-formatted-interpolation.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
ready"""
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-string-interpolation";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-string-interpolation.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-conditionals";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-conditionals.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-match";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-match.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-loops";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-loops.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-comparisons";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-comparisons.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-floating-arithmetic";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-floating-arithmetic.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-sizeof";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-sizeof.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-inherited-methods";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-inherited-methods.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-inherited-fields";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-inherited-fields.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-parent-constructor-args";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-parent-constructor-args.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-super-calls";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-super-calls.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-static-overrides";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-static-overrides.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-virtual-dispatch";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-virtual-dispatch.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-trait-dispatch";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-trait-dispatch.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-abstract-trait";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-abstract-trait.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-abstract-type-members";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-abstract-type-members.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-abstract-type-signatures";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-abstract-type-signatures.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
int smokeBoxedDependentScalarsMvp() {
  ...
  if (int code = expect(
          scalanative::runtime::boxedPrimitiveKind("Unit") ==
              ...
              scalanative::runtime::runtimeAbiName() == "scalanative-runtime-56",
...
object Main {
  ...
  if (int code = expect(
          contains(result.llvmIr, "Runtime ABI = 'scalanative-runtime-56'") &&
              contains(result.llvmIr,
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-boxed-dependent-scalars";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-boxed-dependent-scalars.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-bounded-type-members";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-bounded-type-members.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-interval-type-members";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-interval-type-members.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-path-dependent-types";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-path-dependent-types.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-type-projections";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-type-projections.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-abstract-dependent-reference";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-abstract-dependent-reference.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-abstract-trait-val";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-abstract-trait-val.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-abstract-trait-constructor-val";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-abstract-trait-constructor-val.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-initialized-trait-vals";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-initialized-trait-vals.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-shadowed-trait-vals";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-shadowed-trait-vals.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-trait-vars";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-trait-vars.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-mixed-trait-accessors";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-mixed-trait-accessors.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-shadowed-trait-vars";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-shadowed-trait-vars.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-trait-hierarchy";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-trait-hierarchy.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-trait-composition";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-trait-composition.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-qualified-super";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-qualified-super.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-trait-super-chain";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-trait-super-chain.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-transitive-super";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-transitive-super.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-diamond-linearization";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-diamond-linearization.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-stackable-traits";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-stackable-traits.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-local-typed-dispatch";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() /
      "scalanative-smoke-local-typed-dispatch.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-local-vars";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-local-vars.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-constructor-body";
  const std::filesystem::path outputPath = std::filesystem::temp_directory_path() /
                                           "scalanative-smoke-constructor-body.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-imports";
  const std::filesystem::path outputPath =
      std::filesystem::temp_directory_path() / "scalanative-smoke-imports.out";
  std::filesystem::remove(binaryPath);
  std::filesystem::remove(outputPath);

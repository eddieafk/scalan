...
object Main {
  ...
  const std::filesystem::path cacheDirectory =
      temporary / "scalanative-smoke-build-report-cache";
  const std::filesystem::path outputPath =
      temporary / "scalanative-smoke-build-report.nir";
...
int smokeBuildConfigurationJson() {
  const std::filesystem::path directory =
      std::filesystem::temp_directory_path() / "scalanative-smoke-config";
...
object Main {
  ...
  const std::filesystem::path cacheDirectory =
      ...
      "scalanative-smoke-incremental-cache";
  std::error_code ignored;
  std::filesystem::remove_all(cacheDirectory, ignored);
...
object Main {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path cacheDirectory =
      temporary / "scalanative-smoke-native-object-cache";
  const std::filesystem::path firstBinary =
      temporary / "scalanative-smoke-native-object-first";
  const std::filesystem::path secondBinary =
      temporary / "scalanative-smoke-native-object-second";
  const std::filesystem::path lldFirstBinary =
      temporary / "scalanative-smoke-native-object-lld-first";
  const std::filesystem::path lldSecondBinary =
      temporary / "scalanative-smoke-native-object-lld-second";
  const std::filesystem::path sysrootBinary =
      temporary / "scalanative-smoke-native-object-sysroot";
  const std::filesystem::path invalidSysrootBinary =
      temporary / "scalanative-smoke-native-object-invalid-sysroot";
  const std::filesystem::path invalidSysroot =
      temporary / "scalanative-smoke-missing-sysroot";
  const std::filesystem::path emptySysroot =
      temporary / "scalanative-smoke-empty-sysroot";
  const std::filesystem::path invalidTargetBinary =
      temporary / "scalanative-smoke-native-object-invalid-target";
  const std::filesystem::path incompleteSysrootBinary =
      temporary / "scalanative-smoke-native-object-incomplete-sysroot";
  const std::filesystem::path incompatibleLinkerBinary =
      temporary / "scalanative-smoke-native-object-incompatible-linker";
  const std::filesystem::path staticBinary =
      temporary / "scalanative-smoke-native-object-static";
  const std::filesystem::path output =
      temporary / "scalanative-smoke-native-object.out";
  const std::filesystem::path lldOutput =
      temporary / "scalanative-smoke-native-object-lld.out";
  const std::filesystem::path sysrootOutput =
      temporary / "scalanative-smoke-native-object-sysroot.out";
  const std::filesystem::path staticOutput =
      temporary / "scalanative-smoke-native-object-static.out";
  std::error_code ignored;
  std::filesystem::remove_all(cacheDirectory, ignored);
...
object Main {
  ...
  const std::filesystem::path caughtBinary =
      temporary / "scalanative-smoke-caught-class-cast";
  const std::filesystem::path caughtOutput =
      temporary / "scalanative-smoke-caught-class-cast.out";
  std::error_code ignored;
...
object Main {
  ...
  const std::filesystem::path uncaughtBinary =
      temporary / "scalanative-smoke-uncaught-class-cast";
  const std::filesystem::path uncaughtOutput =
      temporary / "scalanative-smoke-uncaught-class-cast.out";
...
object Main {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path caughtBinary =
      temporary / "scalanative-smoke-caught-null-receiver";
  const std::filesystem::path caughtOutput =
      temporary / "scalanative-smoke-caught-null-receiver.out";
  std::error_code ignored;
  std::filesystem::remove(caughtBinary, ignored);
  std::filesystem::remove(caughtOutput, ignored);
...
object Main {
  ...
  const std::filesystem::path uncaughtBinary =
      temporary / "scalanative-smoke-uncaught-null-receiver";
  const std::filesystem::path uncaughtOutput =
      temporary / "scalanative-smoke-uncaught-null-receiver.out";
  std::filesystem::remove(uncaughtBinary, ignored);
  std::filesystem::remove(uncaughtOutput, ignored);
  binaryOptions.outputPath = uncaughtBinary;
  scalanative::support::DiagnosticEngine uncaughtDiagnostics;
...
object Main {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path caughtBinary =
      temporary / "scalanative-smoke-caught-string-receiver";
  const std::filesystem::path caughtOutput =
      temporary / "scalanative-smoke-caught-string-receiver.out";
  std::error_code ignored;
  std::filesystem::remove(caughtBinary, ignored);
  std::filesystem::remove(caughtOutput, ignored);
...
object Main {
  ...
  const std::filesystem::path uncaughtBinary =
      temporary / "scalanative-smoke-uncaught-string-receiver";
  const std::filesystem::path uncaughtOutput =
      temporary / "scalanative-smoke-uncaught-string-receiver.out";
  std::filesystem::remove(uncaughtBinary, ignored);
  std::filesystem::remove(uncaughtOutput, ignored);
  binaryOptions.outputPath = uncaughtBinary;
  scalanative::support::DiagnosticEngine uncaughtDiagnostics;
...
object Main {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path caughtBinary =
      temporary / "scalanative-smoke-caught-any-receiver";
  const std::filesystem::path caughtOutput =
      temporary / "scalanative-smoke-caught-any-receiver.out";
  std::error_code ignored;
  std::filesystem::remove(caughtBinary, ignored);
  std::filesystem::remove(caughtOutput, ignored);
...
object Main {
  ...
  const std::filesystem::path uncaughtBinary =
      temporary / "scalanative-smoke-uncaught-any-receiver";
  const std::filesystem::path uncaughtOutput =
      temporary / "scalanative-smoke-uncaught-any-receiver.out";
  std::filesystem::remove(uncaughtBinary, ignored);
  std::filesystem::remove(uncaughtOutput, ignored);
  binaryOptions.outputPath = uncaughtBinary;
  scalanative::support::DiagnosticEngine uncaughtDiagnostics;
...
object Main {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path caughtBinary =
      temporary / "scalanative-smoke-caught-null-throw";
  const std::filesystem::path caughtOutput =
      temporary / "scalanative-smoke-caught-null-throw.out";
  std::error_code ignored;
  std::filesystem::remove(caughtBinary, ignored);
  std::filesystem::remove(caughtOutput, ignored);
...
object Main {
  ...
  const std::filesystem::path uncaughtBinary =
      temporary / "scalanative-smoke-uncaught-null-throw";
  const std::filesystem::path uncaughtOutput =
      temporary / "scalanative-smoke-uncaught-null-throw.out";
  std::filesystem::remove(uncaughtBinary, ignored);
  std::filesystem::remove(uncaughtOutput, ignored);
  binaryOptions.outputPath = uncaughtBinary;
  scalanative::support::DiagnosticEngine uncaughtDiagnostics;
...
object Main {
  ...
  const std::filesystem::path binaryPath =
      temporary / "scalanative-smoke-uncaught-throw-active";
  const std::filesystem::path outputPath =
      temporary / "scalanative-smoke-uncaught-throw-active.out";
  std::error_code ignored;
...
object Main {
  ...
  const std::filesystem::path recursiveBinaryPath =
      temporary / "scalanative-smoke-recursive-exception-report";
  const std::filesystem::path recursiveOutputPath =
      temporary / "scalanative-smoke-recursive-exception-report.out";
...
object Main {
  ...
  const std::filesystem::path circularBinaryPath =
      temporary / "scalanative-smoke-circular-exception-cause";
  const std::filesystem::path circularOutputPath =
      temporary / "scalanative-smoke-circular-exception-cause.out";
  ...
  const std::filesystem::path nestedBinaryPath =
      temporary / "scalanative-smoke-nested-throw-active";
  const std::filesystem::path nestedOutputPath =
      temporary / "scalanative-smoke-nested-throw-active.out";
...
object Invalid {
  ...
  if (int code = expect(
          llvm.ok &&
              contains(llvm.llvmIr, "Runtime ABI = 'scalanative-runtime-56'") &&
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path binaryPath =
      temporary / "scalanative-smoke-try-catch-active";
  const std::filesystem::path outputPath =
      temporary / "scalanative-smoke-try-catch-active.out";
  std::error_code ignored;
  std::filesystem::remove(binaryPath, ignored);
  std::filesystem::remove(outputPath, ignored);
...
class MissingGenericChild extends Parent
  ...
  const std::filesystem::path binary =
      temporary / "scalanative-smoke-reference-generics";
  const std::filesystem::path output =
      temporary / "scalanative-smoke-reference-generics.out";
  std::error_code ignored;
...
object InvalidVariance {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path binary =
      temporary / "scalanative-smoke-variance-inheritance";
  const std::filesystem::path output =
      temporary / "scalanative-smoke-variance-inheritance.out";
  std::error_code ignored;
  std::filesystem::remove(binary, ignored);
  std::filesystem::remove(output, ignored);
...
object Main {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path binary =
      temporary / "scalanative-smoke-contextual-abstractions";
  const std::filesystem::path output =
      temporary / "scalanative-smoke-contextual-abstractions.out";
  std::error_code ignored;
  std::filesystem::remove(binary, ignored);
  std::filesystem::remove(output, ignored);
...
object Labels {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path binary =
      temporary / "scalanative-smoke-product-mirror-derivation";
  const std::filesystem::path output =
      temporary / "scalanative-smoke-product-mirror-derivation.out";
  std::error_code ignored;
  std::filesystem::remove(binary, ignored);
  std::filesystem::remove(output, ignored);
...
class Leaf extends Branch
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path binary =
      temporary / "scalanative-smoke-sum-mirror-derivation";
  const std::filesystem::path output =
      temporary / "scalanative-smoke-sum-mirror-derivation.out";
  std::error_code ignored;
  std::filesystem::remove(binary, ignored);
  std::filesystem::remove(output, ignored);
...
object Main {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path binary =
      temporary / "scalanative-smoke-primitive-generics";
  const std::filesystem::path output =
      temporary / "scalanative-smoke-primitive-generics.out";
  std::error_code ignored;
  std::filesystem::remove(binary, ignored);
  std::filesystem::remove(output, ignored);
...
object InvalidInference {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path binary =
      temporary / "scalanative-smoke-generic-inference";
  const std::filesystem::path output =
      temporary / "scalanative-smoke-generic-inference.out";
  std::error_code ignored;
  std::filesystem::remove(binary, ignored);
  std::filesystem::remove(output, ignored);
...
object InvalidExpectedInference {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path binary =
      temporary / "scalanative-smoke-expected-generic-inference";
  const std::filesystem::path output =
      temporary / "scalanative-smoke-expected-generic-inference.out";
  std::error_code ignored;
  std::filesystem::remove(binary, ignored);
  std::filesystem::remove(output, ignored);
...
object Main {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path binary = temporary / "scalanative-smoke-byte-short";
  const std::filesystem::path output =
      temporary / "scalanative-smoke-byte-short.out";
...
object Main {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path binary = temporary / "scalanative-smoke-zone-bytes";
  const std::filesystem::path output =
      temporary / "scalanative-smoke-zone-bytes.out";
  std::error_code ignored;
  std::filesystem::remove(binary, ignored);
  std::filesystem::remove(output, ignored);
...
object Main {
  ...
  const std::filesystem::path temporary = std::filesystem::temp_directory_path();
  const std::filesystem::path binary =
      temporary / "scalanative-smoke-byte-buffer-state";
  const std::filesystem::path output =
      temporary / "scalanative-smoke-byte-buffer-state.out";
  std::error_code ignored;
  std::filesystem::remove(binary, ignored);
  std::filesystem::remove(output, ignored);
...
object Main {
  ...
  const std::filesystem::path plainBinary =
      temporary / "scalanative-smoke-equivalence-plain";
  const std::filesystem::path optimizedBinary =
      temporary / "scalanative-smoke-equivalence-optimized";
  const std::filesystem::path plainOutput =
      temporary / "scalanative-smoke-equivalence-plain.out";
  const std::filesystem::path optimizedOutput =
      temporary / "scalanative-smoke-equivalence-optimized.out";
...
int main() {
  if (std::getenv("SCALANATIVE_SMOKE_TESTS20_ONLY") != nullptr) {
    return runSmokeTests20();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS19_ONLY") != nullptr) {
    return runSmokeTests19();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS18_ONLY") != nullptr) {
    return runSmokeTests18();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS17_ONLY") != nullptr) {
    return runSmokeTests17();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS16_ONLY") != nullptr) {
    return runSmokeTests16();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS15_ONLY") != nullptr) {
    return runSmokeTests15();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS14_ONLY") != nullptr) {
    return runSmokeTests14();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS13_ONLY") != nullptr) {
    return runSmokeTests13();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS12_ONLY") != nullptr) {
    return runSmokeTests12();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS11_ONLY") != nullptr) {
    return runSmokeTests11();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS10_ONLY") != nullptr) {
    return runSmokeTests10();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS9_ONLY") != nullptr) {
    return runSmokeTests9();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS8_ONLY") != nullptr) {
    return runSmokeTests8();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS7_ONLY") != nullptr) {
    return runSmokeTests7();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS6_ONLY") != nullptr) {
    return runSmokeTests6();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS5_ONLY") != nullptr) {
    return runSmokeTests5();
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS4_ONLY") != nullptr) {
    return runSmokeTests4();
  }
  if (int code = runSmokeTests3()) {
    return code;
  }
  if (std::getenv("SCALANATIVE_SMOKE_TESTS3_ONLY") != nullptr) {
    return 0;
  }
  if (int code = runSmokeTests4()) {
    return code;
  }
  if (int code = runSmokeTests5()) {
    return code;
  }
  if (int code = runSmokeTests6()) {
    return code;
  }
  if (int code = runSmokeTests7()) {
    return code;
  }
  ...
  if (int code = runSmokeTests10()) {
    return code;
  }
  if (int code = runSmokeTests11()) {
    return code;
  }
  if (int code = runSmokeTests12()) {
    return code;
  }
  if (int code = runSmokeTests13()) {
    return code;
  }
  if (int code = runSmokeTests14()) {
    return code;
  }
  if (int code = runSmokeTests15()) {
    return code;
  }
  if (int code = runSmokeTests16()) {
    return code;
  }
  if (int code = runSmokeTests17()) {
    return code;
  }
  if (int code = runSmokeTests18()) {
    return code;
  }
  if (int code = runSmokeTests19()) {
    return code;
  }
  if (int code = runSmokeTests20()) {
    return code;
  }
